...
public:
    SefOdjela(string i, string p,  Datum dat, double  s, double bs, Direktor* d) : Radnik(i, p, dat) {satnica = s; brojRadnihSati = bs; nadredjeni =d; }
...
public:
    Zaposlenik(string i, string p, Datum d, double  s,  double bs, SefOdjela* sef)  : Radnik(i, p,d) {
                         satnica = s; brojRadnihSati = bs; nadredjeni = sef; }
...
for(int i(0); i<3;i++) { niz[i] -> ispisiPodatke(); cout<<endl; }
